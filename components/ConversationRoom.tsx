          ...
          callbacks: {
            ...
            onerror: async (e: any) => {
              ...
              if (errMsg.includes('Network error') || errMsg.includes('Requested entity was not found') || errMsg.includes('403')) {
                setError("Neural connection refused: This feature requires a Gemini API key from a paid GCP project. Standard keys do not support low-latency voice synapses.");
              } else {
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6 px-4">
        <div className="p-8 bg-slate-900 border border-red-500/30 rounded-3xl text-center max-w-md shadow-2xl">
          ...
          <div className="flex flex-col gap-3">
            <button onClick={handleKeyError} className="w-full py-3 bg-blue-600 rounded-xl font-bold hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/20">
              Establish Paid Link
            </button>
            <button onClick={onExit} className="w-full py-2 text-slate-500 hover:text-white text-[10px] uppercase font-black tracking-widest transition-colors">
              Return to Labs
            </button>
          </div>
        </div>
